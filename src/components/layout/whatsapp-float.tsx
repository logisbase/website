...
export default function WhatsAppFloat() {
  ...
  return (
    <Link
      ...
      aria-label="Chat with LogisBase on WhatsApp"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      className={[
        // Positioning & stacking — always visible
        'fixed right-8 bottom-8 z-[9999]',
        // Size — desktop
        'h-[72px] w-[72px]',
        // Shrink on small screens
        'max-[768px]:right-5 max-[768px]:bottom-5 max-[768px]:h-14 max-[768px]:w-14',
        // Shape & colour
        ...
        // Accessibility ring
        'focus-visible:ring-4 focus-visible:ring-[#25D366]/50 focus-visible:outline-none',
      ].join(' ')}
    >
      ...
      <svg
        ...
        fill="white"
        aria-hidden="true"
        className="h-[34px] w-[34px] max-[768px]:h-[26px] max-[768px]:w-[26px]"
      >
      ...
      <span
        className={[
          'absolute top-1/2 right-[calc(100%+12px)] -translate-y-1/2',
          'hidden md:block',
          'rounded-lg px-3 py-1.5 whitespace-nowrap',
          'bg-[#111827] text-xs font-medium text-white',
          'shadow-lg',
      ...
      >
        Chat with us
        {/* Arrow */}
        <span className="absolute top-1/2 right-[-5px] h-0 w-0 -translate-y-1/2 border-t-[5px] border-b-[5px] border-l-[5px] border-t-transparent border-b-transparent border-l-[#111827]" />
      </span>
    </Link>
  );
}
