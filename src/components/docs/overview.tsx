...
const QUICK_START_SECTIONS = [
  {
    title: 'Installation',
    description:
      'Get started with LogisBase — install via CLI, Docker, or from source and have your instance running in minutes.',
    icon: Package,
  ...
  },
  {
    title: 'Core Concepts',
    description:
      'Understand the LogisBase architecture: extensions, the console, the API, and how everything fits together.',
    icon: BookOpen,
  ...
  },
  {
    title: 'Webhooks',
    description:
      'Receive real-time event notifications from LogisBase and integrate them into your own systems and workflows.',
    icon: Webhook,
    href: '/docs/webhooks',
  },
  {
    title: 'API Reference',
    description:
      'Full REST API documentation for all LogisBase resources — orders, drivers, places, vehicles, and more.',
    icon: Zap,
    href: '/docs/api',
  },
  {
    title: 'File Systems',
    description:
      'Configure storage adapters for file uploads, media, and document management within LogisBase.',
    icon: FileText,
  ...
  },
  {
    title: 'CLI Reference',
    description:
      'Master the LogisBase CLI for scaffolding extensions, managing deployments, and publishing to the registry.',
    icon: Terminal,
    href: '/docs/cli',
  },
];
  ...
  return (
    <div className="text-center">
      <div className="mx-auto flex max-w-2xl flex-1 flex-col items-center gap-5">
        ...
        <h1 className="!mb-0 text-4xl leading-none text-balance md:text-5xl lg:text-6xl">
          Welcome to the <span className="text-gradient">LogisBase Docs</span>
        </h1>

        <p className="text-muted-foreground !my-0 leading-snug md:text-lg">
          Guides, references, and resources for building with LogisBase
        </p>
      </div>
      ...
      <div className="mx-auto mt-18 grid gap-4 text-start md:mt-21 md:grid-cols-2 lg:grid-cols-3">
  ...
  return (
    <div className="relative text-start">
      ...
      >
        ...
        />
        <Search className="absolute top-1/2 left-6 size-4 -translate-y-1/2" />
        {isExpanded ? (
          <Button
            variant="ghost"
            size="sm"
            onClick={onClose}
            className="text-muted-foreground absolute top-1/2 right-4 -translate-y-1/2 !px-0"
          >
            <X className="h-4 w-4" />
          </Button>
        ) : (
          <kbd className="text-muted-foreground pointer-events-none absolute top-1/2 right-4 inline-flex -translate-y-1/2 transform items-center gap-1 rounded border border-none py-1.5 text-xs select-none">
      ...
      {isExpanded && search.length > 0 && (
        <motion.div
          ...
          className="bg-muted/20 absolute top-full right-0 left-0 z-50 mt-2 max-h-96 overflow-y-auto rounded-md border shadow-lg backdrop-blur-sm"
        >
          <div className="p-2">
            {query.data &&
