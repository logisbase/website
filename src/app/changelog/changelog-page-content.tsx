...
function parseMarkdown(md: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (h6) {
      closeList();
      output.push(`<h6 class="md-h6">${inlineFormat(h6[1])}</h6>`);
      continue;
    }
    if (h5) {
      closeList();
      output.push(`<h5 class="md-h5">${inlineFormat(h5[1])}</h5>`);
      continue;
    }
    if (h4) {
      closeList();
      output.push(`<h4 class="md-h4">${inlineFormat(h4[1])}</h4>`);
      continue;
    }
    if (h3) {
      closeList();
      output.push(`<h3 class="md-h3">${inlineFormat(h3[1])}</h3>`);
      continue;
    }
    if (h2) {
      closeList();
      output.push(`<h2 class="md-h2">${inlineFormat(h2[1])}</h2>`);
      continue;
    }
    if (h1) {
      closeList();
      output.push(`<h1 class="md-h1 sr-only">${inlineFormat(h1[1])}</h1>`);
      continue;
    }
  ...
  if (inCodeBlock && codeLines.length) {
    output.push(
      `<pre class="md-pre"><code class="md-code">${codeLines.map(escapeHtml).join('\n')}</code></pre>`,
    );
  }
...
export default function ChangelogPageContent() {
  ...
  useEffect(() => {
    fetch(
      'https://api.github.com/repos/logisbase/logisbase/releases?per_page=30',
      {
        headers: {
      ...
      <section className="section-padding container max-w-5xl space-y-12">
        ...
        <div className="flex flex-wrap items-center justify-between gap-10">
          <div className="space-y-3">
            ...
            <p className="text-muted-foreground text-lg leading-snug">
              New updates and product improvements to LogisBase.
            </p>
          </div>
        ...
        {!loading && !error && releases.length > 0 && (
          <div className="[--sidebar-width:160px]">
            ...
            <div className="scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent bg-muted/20 relative max-h-[72vh] overflow-y-auto rounded-xl border p-6 pr-2 md:p-8">
              {releases.map((release, index) => (
                <div key={release.id} className="flex gap-5 md:gap-12">
                  ...
                  <div className="relative mt-0.5 shrink-0 md:mt-1.5 md:w-[var(--sidebar-width)]">
                    <time className="text-muted-foreground hidden text-sm md:inline-block">
                  ...
                  <div
                    className={cn(
                      'mb-16 min-w-0 flex-1',
                      index === releases.length - 1 && 'mb-0',
                    )}
                  >
                    {/* Mobile date */}
                    <time className="text-muted-foreground mb-3 inline-block text-sm md:hidden">
                    ...
                    <div className="mb-4 flex flex-wrap items-center gap-3">
                      ...
                      <Link
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-muted-foreground hover:text-foreground hover:border-foreground inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs transition-colors"
                      >
                      ...
                      {release.prerelease && (
                        <span className="rounded-full border border-amber-500/30 bg-amber-500/15 px-2.5 py-1 text-xs font-medium text-amber-600 dark:text-amber-400">
                          Pre-release
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-6">
                      <Link
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-primary inline-flex items-center gap-1.5 text-sm transition-opacity hover:opacity-80"
                      >
