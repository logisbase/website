...
description: Subscribe to LogisBase socket events to display live driver locations and order status updates in your own application or dashboard.
---

# Set Up Real-Time Tracking

LogisBase broadcasts real-time events — driver location updates, order status changes, and platform events — over WebSocket using [SocketCluster](https://socketcluster.io/). This recipe shows how to subscribe to those channels from your own application and render live driver positions on a map.

## What You'll Build

A browser page that connects to the LogisBase socket server, subscribes to your organisation's event channel, and plots incoming driver locations on a Leaflet.js map in real time.

## Prerequisites

- A LogisBase account with FleetOps installed and at least one active driver using the Navigator app
- Your **API public key** and **company UUID** from the Developer Console
- Your socket server hostname (for self-hosted: your domain; for LogisBase Cloud: provided in your account settings)

## How the Socket Layer Works

LogisBase maintains two types of channels you can subscribe to:
...
const socket = socketClusterClient.create({
  hostname: 'socket.your-instance.com', // or your LogisBase Cloud socket host
  secure: true,
  port: 443,
});

socket.on('connect', () => {
  console.log('Connected to LogisBase socket server');
});
...
<Callout type="info">
  SocketCluster automatically reconnects on disconnect. You do not need to
  implement reconnect logic yourself.
</Callout>
...
This example uses [Leaflet.js](https://leafletjs.com/), a lightweight source-accessible mapping library.
...
async function loadActiveDrivers() {
  const response = await fetch(
    'https://api.logisbase.com/v1/drivers?status=active',
    {
      headers: {
...
<Callout type="info">
  Coordinates follow GeoJSON order: `[longitude, latitude]`. Swap them when
  passing to Leaflet's `[lat, lng]` convention.
</Callout>
