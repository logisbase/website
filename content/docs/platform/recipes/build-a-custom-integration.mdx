...
description: Use the LogisBase REST API and webhooks to integrate an external system — push orders in, receive status events out, handle retries safely.
---

# Build a Custom Integration

This recipe shows a complete, production-ready integration pattern: an external system (ERP, WMS, or e-commerce platform) that creates orders in LogisBase via the REST API, then receives real-time status updates back via webhooks.
...
External System
      ...
      ▼
LogisBase REST API ──── FleetOps (dispatch, routing)
      │
...
- A LogisBase account with FleetOps installed
...
When your ERP or WMS generates a delivery task, push it to LogisBase as an order:

```javascript
const FLEETBASE_API = 'https://api.logisbase.com';
const API_KEY = 'flb_live_your_public_key_here';

async function createLogisBaseOrder(externalOrder) {
  ...
  console.log(
    `Created LogisBase order: ${order.public_id} for external ID: ${externalOrder.id}`,
  );
...
Store the relationship between your external order ID and the LogisBase order ID so you can correlate incoming webhook events later:
...
async function syncOrder(externalOrder) {
  const logisbaseOrder = await createLogisBaseOrder(externalOrder);

  // Store both directions
  orderMapping.set(externalOrder.id, logisbaseOrder.id);
  orderMapping.set(logisbaseOrder.public_id, externalOrder.id);

  return logisbaseOrder;
}
```

## Step 4 — Set Up the Webhook Receiver

Register an HTTPS endpoint to receive status events back from LogisBase:
...
app.post('/webhooks/logisbase', async (req, res) => {
  const event = req.body;

  // Acknowledge immediately — don't make LogisBase wait for your processing
  res.sendStatus(200);
...
async function handleStatusUpdate(event) {
  ...
  // Look up your external order from the LogisBase public_id
  ...
  console.log(
    `Order ${order.public_id} (external: ${externalOrderId}) → ${eventType}`,
  );
...
<Steps>
  ...
  <Step>
    ...
    | Event | Meaning |
    | :--- | :--- |
    | `order.created` | Order accepted into LogisBase |
  ...
  <Step>
    Click **Save**. LogisBase will begin delivering to your endpoint immediately.
  </Step>
</Steps>

## Step 6 — Handle Idempotency for Retries

If your receiver returns a non-2xx status or times out, LogisBase will retry the delivery. Your handler must be safe to call multiple times with the same event.
...
```javascript
async function getOrderStatus(logisbasePublicId) {
  const response = await fetch(
    `${FLEETBASE_API}/v1/orders/${logisbasePublicId}`,
    {
...
<Callout type="info">
  Prefer webhooks over polling for real-time updates. Polling introduces latency
  and unnecessary API load. Use polling only as a fallback for reconciliation
  (e.g., a nightly job to catch any missed events).
</Callout>
