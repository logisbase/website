...
description: Self-host LogisBase on AWS, GCP, DigitalOcean, or any Linux server — Docker Compose deployment, Nginx reverse proxy, and SSL setup.
---

# Deploy in Cloud

This guide covers deploying LogisBase to infrastructure you control — a VM on AWS, GCP, DigitalOcean, or any Linux server with Docker installed.

<Callout type="info">
  Prefer not to manage infrastructure yourself? Our **[Managed Installation
  Service](/services/installation)** handles provisioning, deployment, SSL, and
  ongoing maintenance — fully configured and production-ready on your preferred
  cloud provider.
</Callout>
...
<Tabs items={['AWS', 'GCP', 'DigitalOcean', 'VPS / Linux']}>
  <Tab value="AWS">
    The recommended architecture on AWS uses managed services for the database, cache, and storage, with LogisBase running on an EC2 instance.
    ...
    **Recommended AWS services**
    ...
    <Steps>
      ...
      <Step>
        ...
        services:
          application:
            environment:
              ...
              APP_ENV: "production"
              APP_DEBUG: "false"
              DATABASE_URL: "mysql://user:password@your-rds-endpoint/logisbase"
        ...
        ```bash
        docker run --rm logisbase/logisbase-api:latest php artisan key:generate --show
        ```
      </Step>
      <Step>
  ...
  <Tab value="GCP">
    The recommended architecture on GCP uses managed services for the database, cache, and storage, with LogisBase running on a Compute Engine instance.
        ...
        services:
          application:
            environment:
              ...
              APP_ENV: "production"
              APP_DEBUG: "false"
              DATABASE_URL: "mysql://user:password@your-cloud-sql-ip/logisbase"
    ...
    **Recommended DigitalOcean services**
    ...
    <Steps>
      <Step>
        ...
        - **Managed MySQL** — create a MySQL 8 database cluster in your preferred region; create a database named `logisbase` and a user
      ...
      <Step>
        ...
        services:
          application:
            environment:
              ...
              APP_ENV: "production"
              APP_DEBUG: "false"
              DATABASE_URL: "mysql://user:password@your-db-host/logisbase"
        ...
        > DigitalOcean Spaces is S3-compatible, so LogisBase uses the S3 driver. Set `AWS_DEFAULT_REGION` to your Space's region slug (e.g. `nyc3`, `sgp1`, `ams3`).
      </Step>
      <Step>
        ### Start the stack and run migrations
        ...
        services:
          application:
            environment:
              ...
              APP_ENV: "production"
              APP_DEBUG: "false"
              DATABASE_URL: "mysql://logisbase:password@database/logisbase"
...
sudo nano /etc/nginx/sites-available/logisbase
```
...
```bash
sudo ln -s /etc/nginx/sites-available/logisbase /etc/nginx/sites-enabled/
sudo nginx -t
...
<Callout type="info">
  Always run `./deploy.sh` after pulling a new release. It handles migrations,
  cache clearing, and any other steps required by the release.
</Callout>
...
```bash
# /usr/local/bin/logisbase-backup.sh
#!/bin/bash
BACKUP_DIR="/backups/logisbase"
DATE=$(date +%Y%m%d_%H%M%S)
mkdir -p $BACKUP_DIR

docker compose -f /path/to/logisbase/docker-compose.yml \
  exec -T database \
  mysqldump -u root logisbase > $BACKUP_DIR/logisbase_${DATE}.sql

gzip $BACKUP_DIR/logisbase_${DATE}.sql
...
```bash
chmod +x /usr/local/bin/logisbase-backup.sh
echo "0 2 * * * root /usr/local/bin/logisbase-backup.sh" | sudo tee /etc/cron.d/logisbase-backup
```
