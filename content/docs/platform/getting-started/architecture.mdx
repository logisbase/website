---
title: Architecture
description: How LogisBase is structured — the API backend, Ember.js console, real-time engine, extension system, and infrastructure requirements.
---
...
The LogisBase backend is a **Laravel PHP application** structured as a Composer monorepo. The core platform and each extension are distributed as separate Composer packages that are installed into a single Laravel application.

### Core Package: `logisbase/core-api`
...
| Package                    | Provides                                                                      |
| :------------------------- | :---------------------------------------------------------------------------- |
| `logisbase/fleetops-api`   | Orders, drivers, vehicles, fleets, routes, tracking events, proof of delivery |
| `logisbase/storefront-api` | Stores, networks, products, categories, carts, checkout, customers            |
| `logisbase/pallet-api`     | Inventory items, warehouses, stock movements, stock levels                    |
| `logisbase/ledger-api`     | Invoices, ledger entries, financial records                                   |
...
The LogisBase Console is built with **Ember.js** — a JavaScript framework optimised for ambitious, long-running web applications. The console uses a modular architecture where each extension is mounted as an **Ember Engine**.

### Core Libraries

| Package                 | Role                                                            |
| :---------------------- | :-------------------------------------------------------------- |
| `@logisbase/ember-core` | Core services, adapters, utilities, and the Universe Service    |
| `@logisbase/ember-ui`   | Shared component library — buttons, tables, modals, forms, maps |
...
<Files>
  <Folder name="console/" defaultOpen>
    ...
    <Folder
      name="environments/"
      label="Frontend environment config (.env.development, .env.production)"
    />
    <Folder name="lib/" label="Bundled extension engines" />
    <File
      name="logisbase.config.json"
      label="Runtime API and socket configuration"
    />
...
The **Universe Service** (`@logisbase/ember-core`) is the central extensibility registry for the console. When an extension engine initialises, it calls the Universe Service to register its UI elements:
...
LogisBase uses **SocketCluster** — a WebSocket framework for Node.js — to power all real-time features: live driver tracking, order status updates, chat messages, and push-notification triggers.

### Channel Naming

Channels follow the pattern `{resource_type}.{resource_id}`. LogisBase resource IDs are prefixed with the resource type, so a full channel name looks like:
...
You can subscribe to these channels using the LogisBase JS SDK or any SocketCluster-compatible client. The socket server listens on port `38000` and is accessible at the path `/socketcluster/`. Configure and monitor active subscriptions in the [Developer Console → Socket Events](/docs/platform/developer-console/socket-events).

## Extension System

Every LogisBase extension has a **dual architecture** — a backend package and a frontend engine that work together. This is the pattern followed by all first-party extensions and enforced by the Extension Development scaffolding.
...
<Steps>
  ...
  <Step>
    **Frontend engine (Ember)**
    ...
    - Uses `@logisbase/ember-ui` components for consistent styling
  ...
  <Step>
    **Installation and registration**
    ...
    Extensions are installed via the LogisBase CLI (`flb install`) or directly from the Extensions browser in the console. On install:
...
A LogisBase deployment requires three external services in addition to the application containers:
...
LogisBase runs on any cloud that supports Docker containers. Below is a guide to deploying on the four most common cloud providers using their managed services.

<Tabs items={['AWS', 'Azure', 'GCP', 'DigitalOcean']}>
  <Tab value="AWS">
    AWS is well-suited for LogisBase deployments of any scale, from a single EC2 instance to a fully managed, highly available container architecture.
    ...
    **Services used:**
    ...
    For a straightforward single-server deployment, provision an EC2 instance (t3.medium or larger), install Docker, clone the LogisBase repository, and run the interactive install script:
    ...
    Need help deploying on AWS? The [LogisBase Installation Service](/services/installation) handles the full architecture, deployment, and launch for you.

  </Tab>
  <Tab value="Azure">
    Azure provides a complete set of managed services that map cleanly to LogisBase's infrastructure requirements.
    ...
    **Services used:**
    ...
    - **Azure Blob Storage** — S3-compatible object storage (use the Azure SDK or a MinIO gateway for S3 API compatibility with LogisBase's storage driver).
    ...
    For a single-server setup, provision an Azure Linux VM (Standard_D2s_v3 or larger), install Docker, and run the LogisBase install script with your Azure managed service endpoints configured when prompted.

    Need help deploying on Azure? The [LogisBase Installation Service](/services/installation) handles the full architecture, deployment, and launch for you.

  </Tab>
  <Tab value="GCP">
    Google Cloud Platform offers strong managed database and container options for LogisBase deployments.

    **Services used:**
    - **Cloud Run or GKE** — Cloud Run is the simplest path for containerised LogisBase — it runs containers in a serverless environment with automatic scaling. GKE (Kubernetes Engine) is the right choice for operations that need fine-grained control over container orchestration.
    ...
    - **Cloud Storage** — Object storage for files and media, with S3-interoperability enabled for use with LogisBase's S3 storage driver.
    ...
    Need help deploying on GCP? The [LogisBase Installation Service](/services/installation) handles the full architecture, deployment, and launch for you.

  </Tab>
  <Tab value="DigitalOcean">
    DigitalOcean is a straightforward and cost-effective platform for self-hosted LogisBase deployments, particularly for small-to-medium operations.

    **Services used:**
    - **Droplet** — A Linux VPS (4 GB RAM / 2 vCPUs minimum; 8 GB recommended for production). Run the full LogisBase Docker Compose stack directly on the Droplet.
    ...
    - **Spaces** — S3-compatible object storage for file uploads and media. LogisBase's S3 driver works directly with Spaces using the Spaces endpoint and access keys.
    ...
    Need help deploying on DigitalOcean? The [LogisBase Installation Service](/services/installation) handles the full setup, configuration, and launch for you.
...
LogisBase is configured via environment variables written to `docker-compose.override.yml` by the install script. The configuration is split between the **backend API**, the **socket server**, and the **frontend console**.
...
| `APP_NAME`       | Display name for the installation                                          | `LogisBase`                        |
...
| `DATABASE_URL`   | MySQL connection string                                                    | `mysql://user:pass@host/logisbase` |
...
<Callout type="warn">
  In production, always restrict `SOCKETCLUSTER_OPTIONS` origins to your
  console's exact URL. The install script sets this automatically based on the
  host you provide.
</Callout>
...
**`console/logisbase.config.json`** — runtime config loaded by the Ember app:
...
| :--------------------- | :----------------------- | :--------------------------------------------- |
| **Console UI**         | Ember.js + Ember Engines | `@logisbase/ember-core`, `@logisbase/ember-ui` |
| **API Backend**        | Laravel / PHP            | `logisbase/core-api`                           |
...
- [Cloud Quickstart](/docs/platform/quickstart/cloud-quickstart) — get a working LogisBase instance running now
