---
title: API Keys
description: Generate and manage API credentials for authenticating requests to the LogisBase REST API.
---

# API Keys

API keys are the credentials used to authenticate requests to the LogisBase REST API. Each key belongs to either the **Test** or **Live** environment and can be scoped to a specific expiry window. Navigate to **Developers → API Keys** to manage them.
...
```bash
curl -X GET "https://api.logisbase.com/v1/orders" \
