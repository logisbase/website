...
<Callout type="warn">
  Uninstalling an extension may permanently delete the data it managed — for
  example, uninstalling FleetOps removes driver and vehicle records. Back up any
  data you need before proceeding.
</Callout>
...
```bash
flb uninstall logisbase/fleetops

# Target a specific instance directory
flb uninstall logisbase/fleetops --path /opt/logisbase
```
...
| **LogisBase Token** | Your LogisBase account token (click to copy)                             |
