...
description: Browse the LogisBase extension registry and install extensions via the console or the CLI.
---

# Browsing & Installing Extensions

Extensions are discovered and installed from the **Extensions** section in the top navigation bar of the LogisBase Console, or using the `flb` CLI on a self-hosted instance.
...
On a self-hosted instance, use the LogisBase CLI to install extensions. The extension identifier uses the format `publisher/extension-name` — no `@` prefix.

```bash
# Install an extension by name
flb install logisbase/fleetops

# Install to a specific LogisBase instance directory
flb install logisbase/fleetops --path /opt/logisbase
```
...
Before you can install extensions from the CLI you must authenticate with the LogisBase Registry.
