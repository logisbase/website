...
<Callout type="info">
  Organization-level 2FA enforcement only controls access within your
  organization. The available delivery methods (Email or SMS) are determined by
  the system-wide configuration set by the instance administrator.
</Callout>
...
Instance administrators can enable and enforce 2FA across the entire LogisBase instance — for all users in all organizations. This is configured from **Admin → 2FA Config**.
