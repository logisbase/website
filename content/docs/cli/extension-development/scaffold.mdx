---
title: Scaffold
description: 'Generate a new LogisBase extension from the starter template using `flb scaffold`.'
---

# `flb scaffold`

Generate a new LogisBase extension from the [`logisbase/starter-extension`](https://tally.so/r/obgQQx) template. The CLI clones the template, prompts for your extension's name and metadata, then refactors namespaces, package names, and class names so it's ready to develop against immediately.
...
| `-n, --namespace <namespace>`     | PHP namespace (auto-prefixed with `LogisBase\`)         |
...
4. **Refactors namespaces** — every PHP file under `server/` has its `namespace` and `use` statements rewritten from the starter's `LogisBase\Starter\…` to your `LogisBase\<YourName>\…`.
...
| PHP namespace    | `LogisBase\VehicleInspections`               |
...
# Link the addon into your LogisBase console
cd ../logisbase/console
pnpm link ../vehicle-inspections

# Link the API package into your LogisBase api
cd ../api
