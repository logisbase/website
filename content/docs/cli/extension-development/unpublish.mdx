---
title: Unpublish
description: 'Remove a published extension from the LogisBase Extension Registry using `flb unpublish`.'
---
...
<Callout type="warn">
Unpublishing affects every LogisBase instance that has this extension installed. Future `flb install` calls will fail, and `pnpm install` / `composer require` reruns on existing instances will fail to fetch the package.
