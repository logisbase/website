---
title: Bundle Upload
description: 'Upload an existing extension bundle to the LogisBase registry using `flb bundle-upload`.'
---

# `flb bundle-upload`

Upload a `.tar.gz` bundle (created by [`flb bundle`](/docs/cli/extension-development/bundle)) to the LogisBase Extension Registry. Use this when you've already bundled (e.g. as a separate CI step) and just need to ship the archive.
...
2. Your `~/.npmrc` — looking for the line `//registry.logisbase.com/:_authToken=<token>`

If neither is set, the command aborts:

```
Auth token not found for registry https://registry.logisbase.com.
Please provide an auth token using the --auth-token option.
```

To populate `~/.npmrc`, use [`flb login`](/docs/cli/account/login) or `npm login --registry https://registry.logisbase.com`.
