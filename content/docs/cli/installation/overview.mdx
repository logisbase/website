---
title: Install LogisBase
description: 'Install a self-hosted LogisBase instance on your own infrastructure using `flb install-logisbase` — an interactive Docker-based wizard.'
---
...
The `install-logisbase` command is an interactive wizard that clones the LogisBase repository, builds a `docker-compose.override.yml` tailored to your environment, generates secure credentials, starts the containers, and runs the deploy script — all in one go.
...
| `--directory <path>`  | current directory | Where to install LogisBase                                     |
...
<Steps>
  ...
  <Step>
    **Clone the repository** — runs `git clone https://github.com/logisbase/logisbase.git` if `docker-compose.yml` is not already present in the target directory.
  </Step>
  <Step>
  ...
  <Step>
    **Write console config** — `console/logisbase.config.json`, `console/environments/.env.development`, and `console/environments/.env.production` are generated against your host.
  </Step>
  <Step>
...
```bash
flb install-logisbase \
  --host logisbase.mycompany.com \
  --environment production \
  --directory /opt/logisbase
```
...
| `console/logisbase.config.json`         | Console runtime config (API_HOST, SocketCluster)                                                                 |
