---
title: Search
description: 'Browse the LogisBase Extension Registry from the command line with `flb search`.'
---

# `flb search`

Browse and filter the LogisBase Extension Registry from your terminal. Use this to discover available extensions before installing one.
...
| `-h, --host <host>`         | API host (default: `https://api.logisbase.com`)                         |
...
```
🔍 Searching LogisBase Extensions...

Found 1 extension:

Storefront v1.2.0  Free  ↓ 142  [Commerce]
  logisbase/storefront  by LogisBase
  Headless commerce and marketplace platform for LogisBase.
  Install: flb install logisbase/storefront  or flb install ext_xxx
─────────────────────────────────────────────
Use flb install logisbase/<slug> or flb install <extension_id> to install an extension.
...
```
logisbase/storefront	Storefront	v1.2.0	free
logisbase/pallet	Pallet	v0.9.4	$49.00
```
