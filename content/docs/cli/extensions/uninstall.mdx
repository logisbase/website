---
title: Uninstall
description: 'Remove an installed extension from a running LogisBase instance using `flb uninstall`.'
---

# `flb uninstall`

Remove a previously installed extension from a LogisBase instance. The CLI runs `pnpm remove` (in `console/`) and `composer remove` (in `api/`).
...
flb uninstall logisbase/storefront
```

### From anywhere

```bash
flb uninstall logisbase/storefront --path /path/to/logisbase
```
