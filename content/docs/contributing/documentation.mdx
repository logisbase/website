...
The documentation you're reading right now lives in the [`logisbase/logisbase`](https://tally.so/r/obgQQx) repository. Every page is editable. If something is wrong, unclear, or missing — submit a PR.
...
The `/docs/api/*` pages are **auto-generated** from the [postman submodule](https://tally.so/r/obgQQx). Don't edit them directly — your changes will be overwritten on the next build.

To improve the API reference:

- For **endpoint docs, parameters, examples** → edit the YAML files in [`logisbase/postman`](https://tally.so/r/obgQQx) and submit a PR there
...
See [`scripts/README.md`](https://tally.so/r/obgQQx) in the docs repo for the full generator architecture.
