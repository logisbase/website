...
A reference for terms that come up repeatedly in LogisBase translations. The default rule: translate everything that has a clear local equivalent. The exceptions below are documented because they tend to cause confusion.
...
| Term           | Notes                                                                                                                 |
| :------------- | :-------------------------------------------------------------------------------------------------------------------- |
| **LogisBase**  | Always one word, capital F                                                                                            |
...
> ✅ "LogisBase ist eine Source-Accessible-Logistikplattform." (German)
> ✅ "プラットフォームのLogisBaseは…" (Japanese)
...
LogisBase aims for a **professional but plain** tone — not corporate-stiff, not casual.

- **Formal vs. informal "you"**: most locale files default to the polite form (German `Sie`, French `vous`, Spanish `usted`). Stick with this — operators are often using LogisBase in a workplace context.
- **No slang or regional idioms**: copy that reads naturally in Nigerian English shouldn't lose meaning when read by a Mexican operator. Translation is similar — pick the version that's universally understandable in your locale's main reading region.
