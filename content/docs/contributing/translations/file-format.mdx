---
title: File format
description: YAML structure, placeholder syntax, pluralization, and translation conventions for LogisBase locale files.
...
<Callout type="warn">
  1. **Translate values, never keys.** Keys are how the application looks up the
  translation. Changing a key (`new` → `nouveau`) breaks the lookup and the UI
  falls back to the raw key. 2. **Preserve placeholders exactly.** Anything in `
  {curly_braces}` is a runtime variable. Leave the name, the braces, and the
  position intact (you can move it around the sentence — see below).
</Callout>
...
| **Brand names**                              | ❌ No                     | `LogisBase`, `FleetOps`, `Storefront`, `Navigator`                                          |
...
Numerical and Latin-script content stays in its natural direction. Brand names like `LogisBase` stay in Latin script.
...
If you don't know how to translate a key yet, **leave it absent** rather than empty — LogisBase falls back to the English version.
