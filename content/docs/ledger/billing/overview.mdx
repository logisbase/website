...
Reusable templates that define how invoices are rendered to PDF. Templates use LogisBase's core `Template` model with the `invoice` context — they reference invoice variables like `{invoice.number}`, `{invoice.total_amount}`, and `{transaction.reference}`. See [Invoice Templates](/docs/ledger/billing/invoice-templates).
