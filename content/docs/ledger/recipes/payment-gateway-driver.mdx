...
The contract you implement is `LogisBase\Ledger\Contracts\GatewayDriverInterface`. Practically every driver extends `LogisBase\Ledger\Gateways\AbstractGatewayDriver` instead, which provides config helpers, logging, and sensible defaults.
...
The DTOs are `LogisBase\Ledger\DTO\PurchaseRequest`, `RefundRequest`, and `GatewayResponse`.
...
use LogisBase\Ledger\Exceptions\WebhookSignatureException;
...
- Read the existing drivers — `LogisBase\Ledger\Gateways\StripeDriver`, `QPayDriver`, and `CashDriver` are the canonical references
- Use `LogisBase\Ledger\PaymentGatewayManager` if you need to invoke gateways programmatically from outside Ledger
