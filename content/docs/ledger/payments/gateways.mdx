...
<Steps>
  ...
  <Step>
    Set **Environment** to `sandbox` while testing, `live` for production. The
    boolean `is_sandbox` is kept in sync automatically.
  </Step>
...
Register the Ledger webhook URL (shown beneath the user-entered field on the gateway form) in the Stripe Dashboard so Stripe can notify LogisBase of `charge.succeeded`, `charge.failed`, and `charge.refunded` events.
