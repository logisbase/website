...
If you are building a custom merchant terminal (rather than using the LogisBase Console), Storefront exposes internal endpoints for advancing an order through its lifecycle:
...
These are authenticated as a LogisBase user, not via a storefront key.
