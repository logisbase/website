...
This split exists so the frontend can confirm payment with the gateway SDK (which keeps card data out of LogisBase) and Storefront only needs to verify the gateway-confirmed token at capture time.
