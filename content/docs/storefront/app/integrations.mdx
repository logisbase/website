...
These map to the SocketCluster service deployed alongside LogisBase. With these set, an order's status card updates without polling, and (if `MAP_DISPLAY_DRIVERS=true`) drivers are visible on the map.
