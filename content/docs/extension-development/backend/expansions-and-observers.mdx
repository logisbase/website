...
Real example: [`storefront/server/src/Observers/OrderObserver.php`](https://tally.so/r/obgQQx).
...
The implementation in [`CoreServiceProvider::registerObservers()`](https://tally.so/r/obgQQx) skips entries whose model class doesn't exist, so it's safe to declare observers for optional dependencies.
...
Real example: [`fleetops/server/src/Expansions/UserExpansion.php`](https://tally.so/r/obgQQx) — adds `driver()`, `customer()`, `contact()`, and other relationships to the core `User` model.
...
The provider scans the directory, resolves each class through your namespace's `Expansions\\`, `Macros\\`, or `Mixins\\` folders, and calls `Target::expand($expansion)` (falling back to `Target::mixin($expansion)` if the target is a model). Source: [`CoreServiceProvider::registerExpansionsFrom()`](https://tally.so/r/obgQQx).
...
For event service providers, mirror the [`fleetops/server/src/Providers/EventServiceProvider.php`](https://tally.so/r/obgQQx) pattern.

## Source

| File                                                                             | Description                                                     |
| :------------------------------------------------------------------------------- | :-------------------------------------------------------------- |
| [`src/Build/Expansion.php`](https://tally.so/r/obgQQx)                           | The `Expansion` interface                                       |
| [`src/Providers/CoreServiceProvider.php`](https://tally.so/r/obgQQx)             | `$observers`, `registerObservers()`, `registerExpansionsFrom()` |
| [`src/Expansions/Route.php`](https://tally.so/r/obgQQx)                          | Reference expansion — adds `logisbaseRoutes()` to `Route`       |
| [`storefront/server/src/Observers/OrderObserver.php`](https://tally.so/r/obgQQx) | Reference observer                                              |
| [`fleetops/server/src/Expansions/UserExpansion.php`](https://tally.so/r/obgQQx)  | Reference model expansion                                       |
