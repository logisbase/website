---
title: Decorators
description: 'Property decorators from @logisbase/ember-core for cross-engine service injection and lazy data loading: @engineService, @fromStore, and @fetchFrom.'
---

# Decorators

`@logisbase/ember-core` ships three custom property decorators for the patterns Ember's built-in `@service` doesn't cover: pulling a service from another engine, lazily querying the store on first access, and lazily fetching from the API on first access.
...
import engineService from '@logisbase/ember-core/decorators/engine-service';

export default class OrderTrackingLookup extends Component {
  @engineService('@logisbase/fleetops-engine') location;
  @engineService('@logisbase/fleetops-engine') movementTracker;
  // …
}
```

Real example: [`packages/fleetops/addon/components/order-tracking-lookup.js:21`](https://github.com/logisbase/logisbase/blob/main/packages/fleetops/addon/components/order-tracking-lookup.js#L21).
...
| :------------ | :---------------------------------------------------------------------------------------------- |
| `engineName`  | The engine package name (e.g. `'@logisbase/fleetops-engine'`)                                   |
...
```js
import fromStore from '@logisbase/ember-core/decorators/from-store';
...
```js
import fetchFrom from '@logisbase/ember-core/decorators/fetch-from';
...
Real example: [`packages/dev-engine/addon/controllers/webhooks/view.js:25`](https://github.com/logisbase/logisbase/blob/main/packages/dev-engine/addon/controllers/webhooks/view.js#L25).
...
| File                                                                                                                             | Description                        |
| :------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------- |
| [`addon/decorators/engine-service.js`](https://github.com/logisbase/ember-core/blob/main/addon/decorators/engine-service.js)     | Cross-engine service decorator     |
| [`addon/decorators/from-store.js`](https://github.com/logisbase/ember-core/blob/main/addon/decorators/from-store.js)             | Lazy `store.query` decorator       |
| [`addon/decorators/fetch-from.js`](https://github.com/logisbase/ember-core/blob/main/addon/decorators/fetch-from.js)             | Lazy `fetch.get` decorator         |
| [`addon/utils/inject-engine-service.js`](https://github.com/logisbase/ember-core/blob/main/addon/utils/inject-engine-service.js) | Underlying engine-service resolver |
