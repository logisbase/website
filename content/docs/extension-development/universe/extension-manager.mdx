...
export default {
  setupExtension(app, universe) {
    universe.whenEngineLoaded('@logisbase/fleetops-engine', this.registerVroom);
  },

  async registerVroom(fleetopsEngine, universe) {
    const vroomEngine = await universe.extensionManager.ensureEngineLoaded(
...
Real example: [`vroom/addon/extension.js:6`](https://github.com/logisbase/vroom/blob/main/addon/extension.js#L6) — VROOM waits for Fleet-Ops, loads its own engine, and plugs its routing implementation into the Fleet-Ops route-optimization service.
...
```js
const fleetopsEngine = await universe.extensionManager.ensureEngineLoaded(
...
registryService.registerRenderableComponent(
  ...
  new ExtensionComponent('@logisbase/ledger-engine', 'order-invoice'),
);
```
...
```js
universe.extensionManager.unloadEngine('@logisbase/some-engine');
```
...
| File                                                                                                                                             | Description                               |
| :----------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------- |
| [`addon/services/universe/extension-manager.js`](https://github.com/logisbase/ember-core/blob/main/addon/services/universe/extension-manager.js) | `ExtensionManagerService`                 |
| [`addon/contracts/extension-boot-state.js`](https://github.com/logisbase/ember-core/blob/main/addon/contracts/extension-boot-state.js)           | Shared boot-state singleton               |
| [`vroom/addon/extension.js`](https://github.com/logisbase/vroom/blob/main/addon/extension.js)                                                    | `whenEngineLoaded` + `ensureEngineLoaded` |
| [`valhalla/addon/extension.js`](https://github.com/logisbase/valhalla/blob/main/addon/extension.js)                                              | Same pattern — routing-engine integration |
