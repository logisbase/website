...
The [Ledger](https://github.com/logisbase/ledger) extension uses a Laravel [`Manager`](https://laravel.com/api/11.x/Illuminate/Support/Manager.html)-based driver pattern for payment gateways. Stripe, QPay, and Cash drivers ship by default — third-party drivers register via `PaymentGatewayManager::extend()`.

## What a Driver Implements

The contract is `LogisBase\Ledger\Contracts\GatewayDriverInterface`. The abstract base class `AbstractGatewayDriver` covers most of it; you only need to fill in gateway-specific methods.
...
use LogisBase\Ledger\DTO\GatewayResponse;
use LogisBase\Ledger\DTO\PurchaseRequest;
use LogisBase\Ledger\DTO\RefundRequest;
use LogisBase\Ledger\Gateways\AbstractGatewayDriver;
...
use LogisBase\Ledger\PaymentGatewayManager;
...
- [`ledger/server/src/Contracts/GatewayDriverInterface.php`](https://github.com/logisbase/ledger/blob/main/server/src/Contracts/GatewayDriverInterface.php) — full contract
- [`ledger/server/src/Gateways/AbstractGatewayDriver.php`](https://github.com/logisbase/ledger/blob/main/server/src/Gateways/AbstractGatewayDriver.php) — extend this
- [`ledger/server/src/Gateways/StripeDriver.php`](https://github.com/logisbase/ledger/blob/main/server/src/Gateways/StripeDriver.php) — full reference driver with tokenization
- [`ledger/server/src/PaymentGatewayManager.php`](https://github.com/logisbase/ledger/blob/main/server/src/PaymentGatewayManager.php) — registration via `extend()`
- <LaravelApi path="Illuminate/Support/Manager.html">
    Laravel `Manager` API
  </LaravelApi>
  — the upstream driver pattern
