...
The wrapper makes the intent ("we're talking to _our_ API") explicit at the call site without leaking the namespace string everywhere.

## Solution 3 — `setNamespace()` (use sparingly)

`fetch.setNamespace('my-extension')` mutates the _shared_ fetch service to use your namespace as default. **This affects every other call** site too — including unrelated console calls in components from other engines that might run in parallel. Don't reach for this unless you fully own the route lifecycle (e.g., a customer-portal-style isolated app where your extension is the only thing running).
...
- [`addon/services/fetch.js`](https://github.com/logisbase/ember-core/blob/main/addon/services/fetch.js) — service source
