...
Extend `@logisbase/ember-core/adapters/application` and override `namespace`:
...
The base `ApplicationAdapter` already handles auth headers (bearer token), the active organization header, the `X-LogisBase-Sandbox` header, and JSON content negotiation. You're only changing where the requests go.
...
import ApplicationSerializer from '@logisbase/ember-core/serializers/application';
import { EmbeddedRecordsMixin } from '@ember-data/serializer/rest';

export default class WidgetSerializer extends ApplicationSerializer.extend(
...
For the adapter to deserialize correctly, your backend resources need to match Ember Data's expected envelope. The platform's `LogisBaseController` + `JsonResource` pattern handles this for you — see [Routes & Controllers](/docs/extension-development/backend/routes-and-controllers#controllers). The `index` action returns:
...
- [`addon/adapters/application.js`](https://github.com/logisbase/ember-core/blob/main/addon/adapters/application.js) — base adapter source
- [`addon/serializers/application.js`](https://github.com/logisbase/ember-core/blob/main/addon/serializers/application.js) — base serializer source
- [`packages/storefront/addon/adapters/storefront.js`](https://github.com/logisbase/storefront/blob/main/addon/adapters/storefront.js) — reference per-extension adapter
- [`packages/storefront/addon/adapters/`](https://github.com/logisbase/storefront/tree/main/addon/adapters) — re-export pattern across many models
- <EmberDataApi path="modules/@ember-data%2Fadapter">
    Ember Data Adapter
  </EmberDataApi>
  — upstream API docs
- <EmberDataApi path="classes/RESTAdapter">RESTAdapter</EmberDataApi> — what
  `ApplicationAdapter` extends
