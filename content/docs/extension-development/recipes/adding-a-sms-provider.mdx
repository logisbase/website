...
The platform's `LogisBase\Services\SmsService` ships with Twilio (default) and CallPro. Both implement the same minimal contract: a constructor that reads config, plus a `send($to, $text, $options?)` method. Adding a third provider is a matter of writing the sender class, exposing it via config, and (if you want auto-routing) updating the routing rules.
...
```php
$sms = app(\LogisBase\Services\SmsService::class);
...
- [`core-api/src/Services/SmsService.php`](https://github.com/logisbase/core-api/blob/main/src/Services/SmsService.php) — base service with auto-routing
- [`core-api/src/Services/CallProSmsService.php`](https://github.com/logisbase/core-api/blob/main/src/Services/CallProSmsService.php) — reference provider class
- [API Services → SmsService](/docs/extension-development/reference/api-services#logisbase-services-smsservice) — service overview
- <LaravelDocs path="container#binding-singletons">
