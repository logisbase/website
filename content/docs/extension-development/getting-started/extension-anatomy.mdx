...
description: A tour of a scaffolded LogisBase extension — addon/, server/, extension.json, and the key entry points (addon/extension.js, the engine, the service provider).
---
...
```jsonc
{
  "name": "Ledger",
  "version": "0.0.3",
  "description": "Accounting & Invoicing Extension for LogisBase",
  "repository": "https://github.com/logisbase/ledger",
  "license": "Source-accessible-or-later",
  "author": "LogisBase Pte Ltd <hello@logisbase.com>",
  "engine": "package.json",
  "api": "composer.json",
}
```
...
import services from '@logisbase/ember-core/exports/services';
...
The `services` array imported from `@logisbase/ember-core/exports/services` is the canonical list of host services you can inject in your engine (`store`, `fetch`, `notifications`, `current-user`, `session`, `universe`, `crud`, `theme`, etc.).
...
The provider extends `LogisBase\Providers\CoreServiceProvider` — that's where the route helpers, observer auto-registration, expansion auto-loading, and middleware groups come from. See [Service Provider](/docs/extension-development/backend/service-provider).
...
Route::prefix(config('my-extension.api.routing.prefix', 'my-extension'))
    ...
    ->group(function ($router) {
        ...
        // API-key routes
        $router->prefix('v1')->middleware(['logisbase.api'])->group(function ($router) {
            $router->logisbaseRoutes('invoices');
        });

        // Session-authenticated console routes
        $router->prefix('int/v1')->middleware(['logisbase.protected'])->group(function ($router) {
            $router->logisbaseRoutes('invoices');
            $router->logisbaseRoutes('gateways');
        });
    });
```

Notice `$router->logisbaseRoutes('invoices')` — that's the route helper from `core-api`'s `Route` expansion that auto-generates a REST resource set (`index`, `show`, `store`, `update`, `destroy`, plus search/count/bulk-action). See [Routes & Controllers](/docs/extension-development/backend/routes-and-controllers).
...
[`logisbase/ledger`](https://github.com/logisbase/ledger) is a representative full-featured extension — both halves, observers, expansions, dashboard, settings menu items, routes, models, migrations. Read it alongside this section.

## Source Reference

| File                                                                                                                                                    | Description             |
| :------------------------------------------------------------------------------------------------------------------------------------------------------ | :---------------------- |
| [Ledger `extension.json`](https://github.com/logisbase/ledger/blob/main/extension.json)                                                                 | Real manifest           |
| [Ledger `addon/extension.js`](https://github.com/logisbase/ledger/blob/main/addon/extension.js)                                                         | Real `setupExtension`   |
| [Ledger `addon/engine.js`](https://github.com/logisbase/ledger/blob/main/addon/engine.js)                                                               | Real engine declaration |
| [Ledger `server/src/Providers/LedgerServiceProvider.php`](https://github.com/logisbase/ledger/blob/main/server/src/Providers/LedgerServiceProvider.php) | Real provider           |
| [Ledger `server/src/routes.php`](https://github.com/logisbase/ledger/blob/main/server/src/routes.php)                                                   | Real routes file        |
