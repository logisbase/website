---
title: API Reference
description: Complete REST API reference for the LogisBase platform — orders, drivers, places, and more.
---
...
The LogisBase API uses API keys to authenticate requests. You can view and manage your API keys in the [Developer Console](/docs/platform/developer-console/api-keys).
...
```bash
curl https://api.logisbase.com/v1/orders \
...
LogisBase uses conventional HTTP response codes to indicate the success or failure of an API request:

| Code range | Meaning                                                                             |
| :--------- | :---------------------------------------------------------------------------------- |
| `2xx`      | Success                                                                             |
| `4xx`      | Failed given the information provided (missing parameter, validation failure, etc.) |
| `5xx`      | An error with LogisBase's servers (rare)                                            |
...
curl "https://api.logisbase.com/v1/orders?limit=25&offset=50" \
  -H "Authorization: Bearer flb_live_…"

# Or page-based — page * limit acts as the offset
curl "https://api.logisbase.com/v1/orders?limit=25&page=3" \
  -H "Authorization: Bearer flb_live_…"
```
