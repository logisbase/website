---
title: Overview
description: How modals work in @logisbase/ember-ui — the modals-manager service, the built-in layouts, and how to build custom modal components.
---

# Modals

Modals in `@logisbase/ember-ui` are driven by a single service — `modals-manager` — that mounts a component into a portal, manages a stack of nested modals, and exposes a Promise-based API.
...
<div className="not-prose mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
  <Card>
    <CardHeader>
      ...
      <CardDescription className="text-xs">
        The full service API — show, confirm, alert, prompt, bulk, progress,
        process, loader, and stack control.
      </CardDescription>
    </CardHeader>
    <CardContent>
      <a
  ...
  <Card>
    <CardHeader>
      ...
      <CardDescription className="text-xs">
        The built-in layouts — what each looks like and which option fields they
        consume.
      </CardDescription>
    </CardHeader>
    <CardContent>
      <a
        href="/docs/ui/modals/modal-layouts"
        className="text-primary text-sm hover:underline"
      >
        Layouts →
      </a>
