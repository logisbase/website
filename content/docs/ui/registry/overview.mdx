...
LogisBase extensions are isolated by default — each one ships its own components, services, and routes. But many features are richer when extensions can plug UI into each other:
...
Every modern LogisBase extension declares a single `addon/extension.js` that exports an object with a `setupExtension(app, universe)` hook. The console calls this hook once when the extension boots — this is where you register your contributions.
...
menuService.registerSettingsMenuItem(
  new MenuItem({
    ...
    view: 'index',
    component: new ExtensionComponent(
      '@logisbase/customer-portal-engine',
...
```js
import { Hook } from '@logisbase/ember-core/contracts';
...
universe.whenEngineLoaded(
  ...
  async (fleetopsEngine, universe) => {
    const myEngine =
      await universe.extensionManager.ensureEngineLoaded('@my-org/my-engine');
...
`@logisbase/ember-core/contracts` exports the canonical shapes you pass to the registries:
...
These source-accessible extensions all use `addon/extension.js` to register their contributions — read them as reference:

| Extension       | What it registers                                                                  | `addon/extension.js`                                                                                            |
| :-------------- | :--------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------- |
| Customer Portal | Hooks, registries, login menu item, settings menu item                             | [customer-portal/addon/extension.js](https://github.com/logisbase/customer-portal/blob/main/addon/extension.js) |
| Storefront      | Header menu item with shortcuts, dashboard widgets                                 | [storefront/addon/extension.js](https://github.com/logisbase/storefront/blob/main/addon/extension.js)           |
| VROOM           | Renderable settings components, route-optimization provider via `whenEngineLoaded` | [vroom/addon/extension.js](https://github.com/logisbase/vroom/blob/main/addon/extension.js)                     |
| Flespi          | Settings menu, map drawer items, vehicle-detail panels, context-menu commands      | [flespi/addon/extension.js](https://github.com/logisbase/flespi/blob/main/addon/extension.js)                   |
| Ledger          | Invoice template context, settings panels                                          | [ledger/addon/extension.js](https://github.com/logisbase/ledger/blob/main/addon/extension.js)                   |
