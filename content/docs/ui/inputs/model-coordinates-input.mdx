...
description: "<ModelCoordinatesInput> wraps <CoordinatesInput> for binding directly to a LogisBase Ember Data model — auto-updates the record's `location` property and reverse-geocodes the address into the model."
---

# `<ModelCoordinatesInput>`

`<ModelCoordinatesInput>` is a thin wrapper around [`<CoordinatesInput>`](/docs/ui/inputs/coordinates-input) that binds directly to an Ember Data record. When the user moves the map marker, the record's `location` property is updated and LogisBase's reverse-geocoder fills in `street1`, `city`, `country`, etc. on the same record.
...
2. The logisbase reverse-geocoder fills in `place.street1`, `place.city`, `place.province`, `place.country`, `place.postal_code`, etc.
...
<ModelCoordinatesInput
  @model={{this.driver}}
  @locationProperty='current_location'
/>
```
...
| File                                                                                                                                           | Description                                       |
| :--------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------ |
| [`addon/components/model-coordinates-input.hbs`](https://github.com/logisbase/ember-ui/blob/main/addon/components/model-coordinates-input.hbs) | Template — wraps `<CoordinatesInput>`             |
| [`addon/components/model-coordinates-input.js`](https://github.com/logisbase/ember-ui/blob/main/addon/components/model-coordinates-input.js)   | Class — handles model binding and reverse-geocode |
