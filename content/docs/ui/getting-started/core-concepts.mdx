---
title: Core Concepts
description: How ember-ui is structured — components, helpers, modifiers, services, and the conventions that make every LogisBase extension feel native.
---

# Core Concepts

`@logisbase/ember-ui` is a standard Ember addon. If you're new to Ember, the four concepts below are the only ones you need before reading the rest of this section.
...
{{is-uuid this.id}}
{{! true / false }}
{{can-action 'fleet-ops driver' 'delete'}}
{{! permission check }}
```
...
Every LogisBase extension is loaded into the console as an Ember Engine. When the engine boots, it inherits the host application's resolver — and ember-ui's components are part of that resolver. That's why you can use `<Button>` in any extension template without importing anything.
