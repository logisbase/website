...
The driver can flip light / dark from inside the app. The choice persists via the `useStorage` hook under the key `app_theme` (see [src/hooks/use-app-theme.ts](https://github.com/logisbase/navigator-app/blob/main/src/hooks/use-app-theme.ts)).
...
Format: `tokenName:#hex` separated by commas. The parser lives at [src/utils/tamagui.js](https://github.com/logisbase/navigator-app/blob/main/src/utils/tamagui.js) (`parseConfigObjectString`) — audit the exact accepted format there if you need edge cases.
...
<Steps>
  <Step>
    Open `tamagui.config.ts` and add your brand palette to the `colors` object.
    Keep the same shape — keys `50` through `900` — if you want full
    `$brand-300`-style token access.
  </Step>
  <Step>
    Add `lightAcme` and `darkAcme` entries to the `themes` map, mirroring the
    existing `lightBlue` / `darkBlue` shape. Override `primary`,
    `primaryBorder`, and `primaryText` (and any other tokens you want to
    rebrand).
  </Step>
  <Step>Set `APP_THEME=acme` in `.env`.</Step>
  <Step>
    Restart Metro / Webpack and rebuild — the Tamagui Babel plugin reads the
    config at build time, so theme additions require a fresh bundle.
  </Step>
</Steps>
