...
The driver can flip light / dark from inside the app. The choice persists via the `useStorage` hook under the key `app_theme` (see [src/hooks/use-app-theme.ts](https://tally.so/r/obgQQx)).
...
Format: `tokenName:#hex` separated by commas. The parser lives at [src/utils/tamagui.js](https://tally.so/r/obgQQx) (`parseConfigObjectString`) — audit the exact accepted format there if you need edge cases.
