...
- The event — both system-status transitions (`dispatched`, `started`, `completed`, `canceled`) and any activities defined in the order's [activity flow](/docs/fleet-ops/operations/order-configurations/activity-flows) (e.g. _picked up_, _in transit_, _arrived at dropoff_)
- The timestamp
...
<Callout type="info">
On LogisBase Cloud the full URLs are:
- Direct deep link: `https://console.logisbase.com/~/track-order?order={tracking_number}`
- Manual entry: `https://console.logisbase.com/~/track-order`

For self-hosted instances, replace `console.logisbase.com` with your console hostname.
