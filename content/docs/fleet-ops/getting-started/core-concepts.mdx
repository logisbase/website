...
A **Driver** is a person who executes orders using the Navigator mobile app. Each driver record links to a LogisBase user account and stores:
...
**Vendors** are third-party carriers or service providers. Unlike internal drivers, vendors can have their own driver pools and fleets. LogisBase supports **Integrated Vendors** for platforms like Lalamove — where orders can be relayed to the partner's API and quotes fetched automatically.
